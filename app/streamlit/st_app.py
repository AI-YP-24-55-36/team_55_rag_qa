...
def main():
    ...
    if uploaded_file is not None:
        ...
        if valid_df is not None:
            ...
            st.sidebar.subheader("удаление дубликатов и пропусков, предобработка текста и отправка данных на сервер")
            ...
            if uploaded_file is not None:
                ...
                if st.sidebar.button("Очистить и отправить"):
                    ...
                    if response.status_code == 201:
                        log_and_display(f"{response.json()}", level="success", display_func=st.success)
