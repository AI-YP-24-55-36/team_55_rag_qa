...
def validate_csv(uploaded_file):
    """Функция проверка корректности поступаемого датасета"""
    try:
        ...
        if len(df.columns) != 3:
            log_and_display(f"Требуется ровно 3 столбца. Текущее количество: {len(df.columns)}", level="error",
        ...
        for col in df.columns:
            ...
            if non_text_columns:
                log_and_display(f"Следующие столбцы не являются текстовыми: {non_text_columns}", level="error",
                                display_func=st.error)
                return None
        ...
        log_and_display("Файл пуст или не может быть прочитан", level="error", display_func=st.error)
        return None
    except AttributeError:
        log_and_display("Некорректный файл", level="error", display_func=st.error)
        return None
